...
function HomeGuest() {
    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 15) {
          ...
          draft.username.message = "Username cannot exceed 15 characters."
        }
      ...
      case "usernameAfterDelay":
        ...
        if (!draft.username.hasErrors && !action.uniquenessChecked) {
      ...
      case "emailAfterDelay":
        ...
        if (!draft.email.hasErrors && !action.uniquenessChecked) {
      ...
      case "passwordImmediately":
        ...
        if (draft.password.value.length > 15) {
          ...
          draft.password.message = "Password cannot exceed 15 characters."
        }
        return
  ...
  }, [state.username.value]) // Can access 'state' after initialization
  ...
  }, [state.email.value]) // Can access 'state' after initialization
  ...
  }, [state.password.value]) // Can access 'state' after initialization
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          appDispatch({ flashMessage: "Congrats!  Welcome to your new account." })
        } catch (e) {
          console.log("There was a problem: ", e)
        }
      }
  ...
  function handleSubmit(e) {
    ...
    dispatch({ type: "usernameAfterDelay", value: state.username.value, uniquenessChecked: true })
    dispatch({ type: "emailImmediately", value: state.email.value })
    dispatch({ type: "emailAfterDelay", value: state.email.value, uniquenessChecked: true })
    ...
    <Page title="Welcome" wide={true}>
      <div className="row align-items-center">
        <div className="col-lg-7 py-3 py-md-5">
          ...
          <p className="lead text-muted">This is your Friendship Couch, where you'd like to share fresh ideas, non-opinionated thoughts, any riveting information and more. We're not random balls rolling around the concrete. Let's share and connect again. =)</p>
        </div>
